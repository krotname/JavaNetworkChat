...
Сервер по умолчанию слушает порт `1500` только на `127.0.0.1`. Для программного запуска используйте
`ChatServerConfig`: он задаёт bind-адрес, порт, максимальное число клиентов, timeout handshake и
timeout чтения после handshake.
...
Команда запросит token через консоль/stdin: token не попадёт в аргументы процесса.
Новые строки содержат versioned PBKDF2-HMAC-SHA256 hash. Канонические старые строки с быстрым
SHA-256 продолжают работать для миграции, но их следует заменить с помощью `createAccount`.
...
```powershell
$env:NETWORK_CHAT_TLS_KEYSTORE_PASSWORD="changeit"
$env:NETWORK_CHAT_TLS_KEY_PASSWORD="changeit"
...
Для доступа с других машин явно задайте `--bind 0.0.0.0` (или конкретный адрес) и обязательно
включите TLS и accounts. Сертификат должен содержать SAN для имени/адреса, которым пользуется клиент.
...
- `ChatConnection` читает и пишет UTF-8 JSON frames размером не более 8 KiB с общим дедлайном.
...
- При включённой истории сервер атомарно сохраняет `ROOM_TEXT`/`PRIVATE_TEXT` в bounded JSONL,
  мигрирует старые unversioned `TEXT` записи и отправляет последние сообщения комнаты при входе.
